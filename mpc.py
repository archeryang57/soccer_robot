...
class MPC(object):
    ...
    def create_model(self):
        # pyomo.environ.ConcreteModel, 解最優化問題通常用 concrete model
        m = ConcreteModel()
        ...
        # dt 差分間隔, 這裡是步長 
        m.dt = Param(initialize=10, mutable=True)
        m.Lf = Param(initialize=car_length, mutable=True)
        ...
        m.coeffs = Param(RangeSet(0, 3), initialize={0:-0.000458316, 1:0.00734257, 2:0.0538795, 3:0.080728}, mutable=True)
        ...
        return m # .create_instance() # 不允許在constructor使用create_instance
        
    def Solve(self, model, state, coeffs):

        self.iN = model
    
        self.iN.s0.clear()
        self.iN.s0._constructed = False
        self.iN.s0.construct( { 0:state[0], 1:state[1], 2:state[2], 3:state[3], 4:state[4], 5:state[5] } )
        self.iN.coeffs.clear()
        self.iN.coeffs._constructed = False
        self.iN.coeffs.construct({0:coeffs[0], 1:coeffs[1], 2:coeffs[2], 3:coeffs[3]})
        self.iN.f_update.clear()
        self.iN.f_update._constructed = False
        self.iN.f_update.construct()
        self.iN.s0_update.clear()
        self.iN.s0_update._constructed = False
        self.iN.s0_update.construct()
        self.iN.psides_update.clear()
        self.iN.psides_update._constructed = False
        self.iN.psides_update.construct()
